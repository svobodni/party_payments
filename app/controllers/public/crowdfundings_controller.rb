class Public::CrowdfundingsController < ApplicationController
  ...
  def index
    ...
    @og_title="Přispějte na kampaň Svobodných!"
    @og_url=public_crowdfundings_url
    @og_description="Namixujte kampaň Svobodných podle svých představ a pomozte nám uspět ve volbách. Můžeme být svobodnou zemí."
    @og_image=ActionController::Base.helpers.image_url "crowdfunding/9907.jpg"
  end

  def show
    @crowdfunding = Crowdfunding.find(params[:id])
    @og_title="Přispějte na kampaň Svobodných! #{@crowdfunding.title}"
