class DonationAgreementPdf < Prawn::Document
  def initialize(donation)
    ...
    text('Darovací smlouva – finanční dar', {
      :size => 24,
      :styles => [:bold],
      :align => :center
    })
    ...
    table([
      ...
      ["2.","Dárce poskytne obdarovanému na jeho činnost finanční dar v hodnotě #{number_to_currency(donation.amount)} (dále jen dar). Dar bude obdarovanému splacen na jeho účet vedený u Fio banky č.#{Prawn::Text::NBSP}ú.:#{Prawn::Text::NBSP}7505075050/2010 a Obdarovaný dar přijímá."],
